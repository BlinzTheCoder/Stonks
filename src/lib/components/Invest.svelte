...
{#if currentStonk}
	<div class="invest-container">
		...
		<div class="actions">
			<button class="invest-actions" onclick={() => invest('up')}>↑</button>
			<button class="invest-actions" onclick={() => invest('down')}>↓</button>
		</div>
	</div>
{/if}
...
<style>
	.invest-container {
		...
		overflow: hidden; /* optional, to prevent outer scroll */
	}
	...
	/* sparkline graph centered */
	.sparkline {
		display: flex;
		align-items: center; /* center bars on vertical midline */
		justify-content: center;
		position: absolute;
		top: 50%;
		left: 50%;
